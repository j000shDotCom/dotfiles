...
complete -c objdump -s M -l disassembler-options -d 'Pass target-specific disassembler options'
...
complete -c objdump -l x86-asm-syntax -d 'When used with \\%--disassemble, choose style of code to emit from X86 backend'
...
complete -c objdump -l disassemble-functions -d 'Disassemble only the specified symbols'
...
complete -c objdump -l version -d 'Display the version of the llvm-objdump executable'
...
complete -c objdump -l cfg -d 'Create a CFG for every symbol in the object file and write it to a graphviz f…'
