# curl
# Autogenerated from man page /Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/share/man/man1/curl.1
complete -c curl -l abstract-unix-socket -d '(HTTP) Connect through an abstract Unix domain socket, instead of using the n…'
complete -c curl -l alt-svc -d '(HTTPS) WARNING: this option is experiemental.  Do not use in production'
...
complete -c curl -l http2 -d '(HTTP) Tells curl to use HTTP version 2.   See also --no-alpn'
